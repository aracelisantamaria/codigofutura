// src/lib.rs
//Contract principal + trait implementation
...
impl TokenTrait for TokenBDB {
    ...
    fn burn(env: Env, from: Address, amount: i128) -> Result<(), TokenError> {
        ...
        // Validación que PIDE PRESTADO (no toma ownership)
    ...
    fn balance(env: Env, account: Address) -> i128 {
        env.storage().persistent()
            .get(&DataKey::Balance(account)) // Prestamos account (no lo movemos)
            .unwrap_or(0) // Si no existe → 0
    }
    
    fn transfer(
        env: Env, 
        from: Address, // from es el dueño de esta Address
        to: Address, // to es el dueño de esta Address
        amount: i128
    ) -> Result<(), TokenError> {
        // 1. Verificar inicialización
        if !env.storage().instance().has(&DataKey::Initialized) {
        ...
        // 3. Validaciones
        if amount <= 0 {
            return Err(TokenError::InvalidAmount); // ❌ Error específico
        }
        ...
        // Si queremos usar from/to múltiples veces, necesitamos clonarlo
        // Clonamos porque necesitamos usar 'from' después
        let from_balance = Self::balance(env.clone(), from.clone()); // Clone para dar ownership
        if from_balance < amount {
            return Err(TokenError::InsufficientBalance); // ❌ Otro error
        }
    ...
    fn transfer_from(
        ...
        env.events().publish(
            (symbol_short!("trnsfr_fm"), spender, from.clone(), to.clone()),
